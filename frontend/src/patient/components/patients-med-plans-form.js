...
class PacientMedsForm extends React.Component {
    ...
    viewMedPlans(id) {
        return API_USERS.getPatientsMeds(id, (result, status, error) => {
            if (result !== null && (status === 200 || status === 201)) {
                this.setState({
                    tableData2: result,
                    isLoaded1: true
                });
                console.log("med planurile pacientului cu id: " + id + " sunt:" + this.state.tableData2);
            } else {
                this.setState(({
                    errorStatus: status,
                    error: error
...
export default PacientMedsForm;
